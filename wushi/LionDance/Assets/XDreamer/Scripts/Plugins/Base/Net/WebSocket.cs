...
namespace XCSJ.Extension.Base.Net
{
    ...
    public class WebSocket
    {
        ...
        public static string ToIP(string url)
        {
            ...
            return hostInfo?.AddressList?[0]?.ToString() ?? url;
#endif
        }
        ...
        public void Connect()
        {
            m_Socket = new WebSocketSharp.WebSocket(mUrl.ToString(), protocols);
            ...
            {
                m_IsConnected = false;
                if (string.IsNullOrEmpty(m_Error)) m_Error = "closed " + e.Code;
            };
