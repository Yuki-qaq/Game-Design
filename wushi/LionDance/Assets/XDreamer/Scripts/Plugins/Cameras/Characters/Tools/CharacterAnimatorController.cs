...
namespace XCSJ.PluginCameras.Characters.Tools
{
    ...
    [Tool(CharacterCategory.Title, nameof(XCharacterController))]
    public class CharacterAnimatorController : BaseCharacterToolController, IAnimatorParameter
    {
        ...
        /// </summary>
        [Name("前进动画速度")]
        [Tip("用于调节前进动画播放速度的量。值越大播放速度越快")]
        [Min(0)]
        public float _forwardMotionSpeed = 1;
        ...
        /// </summary>
        [Name("前进动画速度")]
        [Tip("用于调节转向动画播放速度的量。值越大播放速度越快")]
        [Min(0)]
        public float _turnMotionSpeed = 1;
        ...
        protected virtual void Update()
        {
            ...
            if (!string.IsNullOrEmpty(_forward)) animator.SetFloat(_forward, forwardAmount * _forwardMotionSpeed, 0.1f, deltaTime);
            if (!string.IsNullOrEmpty(_turn)) animator.SetFloat(_turn, Mathf.Atan2(move.x, move.z) * _turnMotionSpeed, 0.1f, deltaTime);
