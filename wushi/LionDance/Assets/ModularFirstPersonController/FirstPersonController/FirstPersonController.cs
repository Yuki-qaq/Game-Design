...
public class FirstPersonController : MonoBehaviour
{
    ...
    public bool useWalkVariable = false;
    public FloatVariableScriptable walkVariable;
    ...
    public bool useSprintVariable = false;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = useInputVariable ? inputVariable.Value : new Vector3(Input.GetAxis(horizontalMoveAxis), 0, Input.GetAxis(verticalMoveAxis));

            if (rotateMoveInputs)
            {
                targetVelocity = Quaternion.AngleAxis(inputRotationVariable.Value.y, Vector3.up) * targetVelocity;
            }
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                {
                    isSprinting = targetVelocity != Vector3.zero;
            ...
            else
            {
                ...
                if (hideBarWhenFull && sprintBarCG && sprintRemaining == sprintDuration)
                {
...
public class FirstPersonControllerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        */
        ...
        fpc.useWalkVariable = EditorGUILayout.Toggle(new GUIContent("Use Walk Variable", "Use an asset file containing the player's speed"), fpc.useWalkVariable);
        if (fpc.useWalkVariable)
        {
            fpc.walkVariable = EditorGUILayout.ObjectField(new GUIContent($"Walk Speed {(fpc.walkVariable ? ("= " + fpc.walkVariable.InitialValue) : string.Empty)}",
                                                                          "Determines how fast the player will move while walking."), fpc.walkVariable, typeof(FloatVariableScriptable), false) as FloatVariableScriptable;
        }
        else
        {
            fpc.walkSpeed = EditorGUILayout.Slider(new GUIContent("Walk Speed", "Determines how fast the player will move while walking."), fpc.walkSpeed, .1f, fpc.sprintSpeed);
        }
        fpc.rotateMoveInputs = EditorGUILayout.Toggle(new GUIContent("Use Input Rotation Variable", "Use an asset file containing the rotation to apply on the move inputs"), fpc.rotateMoveInputs);
        if (fpc.rotateMoveInputs)
        {
            fpc.inputRotationVariable = EditorGUILayout.ObjectField(new GUIContent("Input rotation", "Rotates the Horizontal and Vertical inputs using the provided rotation variable (ex. Camera orientation)"),
                                                                    fpc.inputRotationVariable,
                                                                    typeof(Vector3VariableScriptable),
                                                                    false) as Vector3VariableScriptable;
        }
        fpc.useInputVariable = EditorGUILayout.Toggle(new GUIContent("Use Input Variable", "Enable input from external source (touch controls)"), fpc.useInputVariable);
        if (fpc.useInputVariable)
        {
            fpc.inputVariable = EditorGUILayout.ObjectField(new GUIContent("External Input Variable",
                                                                          "Get the input from a virtual joystick instead of a normal input"), fpc.inputVariable, typeof(Vector3VariableScriptable), true) as Vector3VariableScriptable;
        }
        else
        {
            fpc.horizontalMoveAxis = EditorGUILayout.TextField(new GUIContent("Horizontal Move Axis", "Name of the axis from the input manager for horizontal movement"), fpc.horizontalMoveAxis);
            fpc.verticalMoveAxis = EditorGUILayout.TextField(new GUIContent("Horizontal Move Axis", "Name of the axis from the input manager for vertical movement"), fpc.verticalMoveAxis);
        }
        GUI.enabled = true;
        ...
        fpc.useSprintVariable = EditorGUILayout.Toggle(new GUIContent("Use Sprint Variable", "Use an asset file containing the player's sprint speed."), fpc.useSprintVariable);
        if (fpc.useSprintVariable)
        {
            fpc.sprintVariable = EditorGUILayout.ObjectField(new GUIContent($"Sprint Speed {(fpc.sprintVariable ? ("= " + fpc.sprintVariable.InitialValue) : string.Empty)}",
                                                                            "Determines how fast the player will move while sprinting."), fpc.sprintVariable, typeof(FloatVariableScriptable), false) as FloatVariableScriptable;
        }
        else
        {
        ...
        fpc.sprintDuration = EditorGUILayout.Slider(new GUIContent("Sprint Duration", "Determines how long the player can sprint while unlimited sprint is disabled."), fpc.sprintDuration, 0.1f, 20f);
        fpc.sprintCooldown = EditorGUILayout.Slider(new GUIContent("Sprint Cooldown", "Determines how long the recovery time is when the player runs out of sprint."), fpc.sprintCooldown, .1f, 20f);
        //GUI.enabled = true;
