...
namespace XCSJ.EditorXGUI.Tools
{
    ...
    public static class ToolsMenu
    {
        ...
        [RequireManager(typeof(XGUIManager))]
        [Manual(typeof(Canvas))]
        public static void CreateMainMenu(ToolContext toolContext)
        {
            EditorToolsHelperExtension.FindOrCreateRootAndGroup(toolContext, LoadPrefab_DefaultXGUIPath("主菜单控制器.prefab"));
        }
        ...
        [RequireManager(typeof(XGUIManager))]
        [Manual(typeof(UGUIWindow))]
        public static void CreateWindowTemplate(ToolContext toolContext)
        {
        ...
        /// <param name="toolContext"></param>
        /// <returns></returns>       
        [Tool(XGUICategory.Window, rootType = typeof(Canvas), needRootParentIsNull = true, groupRule = EToolGroupRule.None)]
        [XCSJ.Attributes.Icon(EIcon.Window)]
        [Name("标签窗口模版")]
        [Tip("可通过标签动态切换不同的控制面板", "Includes dynamic list mode and dynamic key value mode")]
        [RequireManager(typeof(XGUIManager))]
        [Manual(typeof(UGUIWindow))]
        public static void CreateLabelWindowTemplate(ToolContext toolContext)
        {
            MenuHelper.DrawMenu("标签窗口模版", m =>
            {
                m.AddMenuItem("水平布局", () =>
                {
                    EditorToolsHelperExtension.FindOrCreateRootAndGroup(toolContext, LoadPrefab_DefaultXGUIPath("水平布局标签窗口模版.prefab"));
                });
                m.AddMenuItem("垂直布局", () =>
                {
                    EditorToolsHelperExtension.FindOrCreateRootAndGroup(toolContext, LoadPrefab_DefaultXGUIPath("垂直布局标签窗口模版.prefab"));
                });
            });
        }
