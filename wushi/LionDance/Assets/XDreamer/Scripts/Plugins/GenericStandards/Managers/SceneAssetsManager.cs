...
namespace XCSJ.PluginGenericStandards.Managers
{
    /// <summary>
    /// 场景资源管理类;用于负责场景调度
    /// </summary>
    public static class SceneAssetsManager
    {
        ...
        /// <summary>
        /// 使用 XDreamer.Root ，在场景切换后,游戏对象销毁，导致本函数不执行！
        ...
        /// <param name="action"></param>
        public static bool AsynImportAndAdditiveLoadScene(string uri, Action<GameObject> action)
        {
            if (string.IsNullOrEmpty(uri)) return false;
            return asyncMono.StartCoroutine(_AsynImportAndAdditiveLoadScene(uri, go =>
            {
                try
                {
                    if (go)
        ...
        static readonly Dictionary<string, AssetBundle> abCache = new Dictionary<string, AssetBundle>();

        static bool inDoing = false;

        private static IEnumerator _AsynImportAndAdditiveLoadScene(string uri, Action<GameObject> action)
        {
            yield return null;
            while (inDoing)
            {
            ...
            if (!abCache.TryGetValue(uri, out assetBundle) || !assetBundle)
            {
                ...
                var download = request.downloadHandler as DownloadHandlerAssetBundle;
                if (download == null)
                {
                    action?.Invoke(default);
                    yield break;
            ...
            if (assetBundle.isStreamedSceneAssetBundle)
            {
                ...
                var actinveScene = SceneManager.GetActiveScene();

                //将累加场景中所有游戏对象转移到主场景，并且全部转移到以累加场景名为名称的游戏对象子级下；
                var root = new GameObject(name);//作为累加场景的根对象
                root.XSetActive(false);
                root.transform.XResetPRS();//保证累加场景的根对象在世界坐标原点
                var srcgos = scene.GetRootGameObjects();
                foreach (var go in srcgos)
                {
                    SceneManager.MoveGameObjectToScene(go, actinveScene);
                    go.XSetParent(root);
                }
