...
namespace XCSJ.EditorZVR.Tools
{
    ...
    public static class ToolsMenu
    {
        ...
        private static CameraController CreateZVRCamera() => XRSpaceSolutionHelper.CreateHMD<CameraTransformByZVR>("HMD-ZVR型平移飞行相机");
        ...
        private static MonoBehaviour CreateXRRig(out CameraController hmd, out Transform leftHand, out Transform rightHand)
        {
            ...
            if (leftHand)
            {
                var pose = leftHand.XAddComponent<PoseByZVR>();
                if (pose) pose.rigidBodyID = 1;
            }

            if (rightHand)
            {
                var pose = rightHand.XAddComponent<PoseByZVR>();
