...
namespace XCSJ.PluginSamsungWMR.Tools
{
    ...
    /// 基于三星玄龙手柄的捕捉转动提供者
    /// </summary>
    [AddComponentMenu("XR/Locomotion/Snap Turn Provider (三星玄龙手柄)")]
    [Name("基于三星玄龙手柄的捕捉转动提供者")]
    [Tool(XRITCategory.LocomotionSystem, nameof(AnalogLocomotionProvider))]
    [Tip("在当前游戏对象上创建一个[基于三星玄龙手柄的捕捉转动提供者]组件对象", "Create a [capture turn provider based on Samsung Xuanlong handle] component object on the current game object")]
    [XCSJ.Attributes.Icon(EIcon.Rotate)]
    [RequireManager(typeof(SamsungWMRManager))]
    [Owner(typeof(SamsungWMRManager))]
    public class SamsungWMRSnapTurnProvider
#if XDREAMER_XR_INTERACTION_TOOLKIT
