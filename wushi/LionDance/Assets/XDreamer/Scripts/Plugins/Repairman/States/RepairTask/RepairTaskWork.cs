...
namespace XCSJ.PluginRepairman.States.RepairTask
{
    ...
    public sealed class RepairTaskWork : StepGroupRoot
    {
        ...
        /// 包含维修步骤
        /// </summary>
        /// <param name="repairStep"></param>
        /// <returns></returns>
        public bool ContainsRepairStep(RepairStep repairStep) => repairSteps.Contains(repairStep);
        ...
        /// <returns></returns>
        public bool ContainPart(Part part) => parts.Contains(part) || modules.Exists(m => m.ContainPart(part));

        private void GetPartOrModuleInStep()
        {
            parts.Clear();
            modules.Clear();
            repairSteps.Clear();
