...
namespace XCSJ.EditorOptiTrack.Tools
{
    ...
    public static class ToolsMenu
    {
        ...
        private static CameraController CreateOptiTrackCamera() => XRSpaceSolutionHelper.CreateHMD<CameraTransformByOptiTrack>("HMD-OptiTrack型平移飞行相机");
        ...
        private static MonoBehaviour Create(out CameraController hmd, out Transform leftHand, out Transform rightHand)
        {
            ...
            if (leftHand)
            {
                var pose = leftHand.XAddComponent<PoseByOptiTrack>();
                if (pose) pose.rigidBodyID = 1;
            }

            if (rightHand)
            {
                var pose = rightHand.XAddComponent<PoseByOptiTrack>();
