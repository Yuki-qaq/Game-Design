...
namespace XCSJ.EditorExtension.Base.Kernel
{
    ...
    public class DefaultEditorHandler : InstanceClass<DefaultEditorHandler>, IEditorHandler
    {
        ...
        class Dictionary : Dictionary<object, object> { };

        class VarStringSO : SO, IDropdownPopupAttribute, IPropertyPathList
        {
            /// <summary>
            /// 属性路径列表
            /// </summary>
        ...
        static VarStringSO varStringSO;
        static SerializedObject varStringSerializedObject;
        static SerializedProperty propertyPathListSerializedProperty;
        ...
        public void DrawVarStringWindowContent(VarStringEditorWindow varStringEditorWindow)
        {
            ...
            if (!varStringSO)
            {
                varStringSO = ScriptableObject.CreateInstance<VarStringSO>();
                varStringSerializedObject = new SerializedObject(varStringSO);
                propertyPathListSerializedProperty = varStringSerializedObject.FindProperty(nameof(VarStringSO._propertyPathList));
            }
            varStringSO.varName = result.varName;
            varStringSO.varScope = result.varScope;
            varStringSerializedObject.UpdateIfRequiredOrScript();

            if (VarStringEditorWindow.newVarString)
            {
                var pathSP = propertyPathListSerializedProperty.FindPropertyRelative(nameof(VarStringSO._propertyPathList._propertyPaths));
            ...
            Action delayCall = () =>
            {
                var newVarString = ScriptHelper.VarFlag + result.varName + "." + varStringSO._propertyPathList.ToPropertyPath() + result.varScope.ToVarScopeString();
            ...
            {
                varStringSO._propertyPathList.SetInstance(value?.GetType(), varStringSO, value);
            }
                ...
                default://下拉选择的绘制
                    {
                        ...
                        if (GUILayout.Button(CommonFun.TempContent(display, display), EditorObjectHelper.MiniPopup))
                        {
                            ...
                            var spCopy = propertyPathListSerializedProperty.Copy();
                            EditorHelper.DrawMenu(selectText, varStrings.ToArray(), newSelectText =>
                            {
                                ...
                                var pathSP = spCopy.FindPropertyRelative(nameof(VarStringSO._propertyPathList._propertyPaths));
            ...
            EditorGUILayout.PropertyField(propertyPathListSerializedProperty, PropertyData.GetPropertyData(propertyPathListSerializedProperty).trLabel);
            ...
            //应用修改
            varStringSerializedObject.ApplyModifiedProperties();
        }
    }
}
