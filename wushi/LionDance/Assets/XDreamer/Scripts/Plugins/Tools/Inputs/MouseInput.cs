...
namespace XCSJ.PluginTools.Inputs
{
    ...
    public sealed class MouseInput : RayInput<MouseCmd, MouseCmds>
    {
        ...
        public class MouseCmd : RayCmd
        {
            ...
            public MouseCmd(EMouseButton mouseButton, string cmdName, EUIRule uiRule = EUIRule.InvalidOnAnyUI, EPressState pressState = EPressState.Pressed, EInputMode inputMode = EInputMode.Standard | EInputMode.Analog, float pressMaxDistance = 10, float fixedTime = 3)
            {
        ...
        public override void ResetCmds()
        {
            ...
            CreateDefaultMouseCmd();            
        }

        private void CreateDefaultMouseCmd()
        {
            this.XModifyProperty(() =>
            {
                var cmds = _cmds._cmds;
                cmds.Add(new MouseCmd(EMouseButton.Left, "左键按下", EUIRule.InvalidOnAnyUIOnlyStandard));
                cmds.Add(new MouseCmd(EMouseButton.Left, "左键按下保持", EUIRule.None, EPressState.Keeping));
                cmds.Add(new MouseCmd(EMouseButton.Left, "左键弹起", EUIRule.None, EPressState.Released));
                cmds.Add(new MouseCmd(EMouseButton.Left, "左键按下并弹起", EUIRule.InvalidOnAnyUIOnlyStandard, EPressState.PressedAndReleased, EInputMode.Standard | EInputMode.Analog, 1f));
                cmds.Add(new MouseCmd(EMouseButton.Left, "碰撞体点击", EUIRule.InvalidOnAnyUIOnlyStandard, EPressState.PressedAndReleased, EInputMode.Standard | EInputMode.Analog, 1f));
                cmds.Add(new MouseCmd(EMouseButton.Any, "鼠标任意动作", EUIRule.InvalidOnAnyUIOnlyStandard, EPressState.NoKeeping));
                cmds.Add(new MouseCmd(EMouseButton.Right, "右键按下", EUIRule.InvalidOnAnyUIOnlyStandard));
                cmds.Add(new MouseCmd(EMouseButton.Right, "右键按下保持", EUIRule.InvalidOnAnyUIOnlyStandard, EPressState.Keeping));
                cmds.Add(new MouseCmd(EMouseButton.Right, "右键弹起", EUIRule.InvalidOnAnyUIOnlyStandard, EPressState.Keeping));
            });
        }
    }
