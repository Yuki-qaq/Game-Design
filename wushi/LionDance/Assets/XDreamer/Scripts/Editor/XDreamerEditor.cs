...
namespace XCSJ.EditorExtension
{
    ...
    public class XDreamerEditor : AssetPostprocessor, IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        /// 文档手册
        /// </summary>
        [MenuItem(XDreamerMenu.NamePath + nameof(XDreamer) + "文档手册", priority = 7500)]
        public static void DocumentManual() => CommonFun.OpenManual(null);
        ...
        public void OnPostprocessBuild(BuildReport report)
        {
            ...
            Debug.Log("编译结束: " + endTime.ToDefaultFormat()+ ", 总时长: " + (endTime - beginTime).TotalSeconds + " 秒");
        ...
        /// 当打包之后
        /// </summary>
        /// <param name="buildTarget">打包目标：打包的目标平台</param>
        /// <param name="pathToBuiltProject">构建项目的路径</param>
        [PostProcessBuild(1)]
        public static void OnAfterBuild(BuildTarget buildTarget, string pathToBuiltProject)
        {
            foreach (var config in XDreamerExtensionOption.weakInstance.postProcessBuildConfigs.Where(c => c.buildTarget == buildTarget))
            {
                OnAfterBuild(buildTarget, pathToBuiltProject, config);
            }
        }

        private static void OnAfterBuild(BuildTarget buildTarget, string pathToBuiltProject, PostProcessBuildConfig config)
        {
            if (config == null) return;

            switch (config.postProcessBuildRule)
            {
                case EPostProcessBuildRule.StartBuiltProject:
                    {
                        //打开文件或文件夹
                        System.Diagnostics.Process.Start(pathToBuiltProject);
                        Debug.Log(config.postProcessBuildRule.Tr() + "完成：" + pathToBuiltProject);
                        break;
                    }
                case EPostProcessBuildRule.PackagingBuiltProjectFolderAsZip:
                    {
                        switch (buildTarget)
                        {
                            case BuildTarget.WebGL:
                                {
                                    var dir = pathToBuiltProject;
                                    if (File.Exists(dir))
                                    {
                                        var fileName = Path.GetFileName(dir);
                                        if ("index.html" != fileName)
                                        {
                                            Debug.LogWarning("未找到有效的【index.html】文件，" + config.postProcessBuildRule.Tr() + "失败！构建项目的路径：" + dir);
                                            return;
                                        }
                                    ...
                                    PackagingBuiltProjectFolderAsZip(dir, pathToBuiltProject);
                                    break;
                                }
                            case BuildTarget.StandaloneWindows:
                                {
                                    ...
                                    {
                                        var fileName = Path.GetFileName(dir);
                                        if (PlayerSettings.productName + ".exe" != fileName)
                                        {
                                            Debug.LogWarning("未找到有效的【" + PlayerSettings.productName + ".exe】文件，" + config.postProcessBuildRule.Tr() + "失败！构建项目的路径：" + dir);
                                            return;
                                        }
                                        dir = Path.GetDirectoryName(dir);
                                    }
                                    ...
                                    {
                                        if (!File.Exists(dir + PlayerSettings.productName + ".exe"))
                                        {
                                            Debug.LogWarning("未找到有效的【" + PlayerSettings.productName + ".exe】文件，" + config.postProcessBuildRule.Tr() + "失败！！构建项目的路径：" + dir);
                                            return;
                                        }
        ...
        private static void PackagingBuiltProjectFolderAsZip(string floderPath, string pathToBuiltProject)
        {
            if (!Directory.Exists(floderPath))
            {
                Debug.LogWarning("目录不存在，" + EPostProcessBuildRule.PackagingBuiltProjectFolderAsZip.Tr() + "失败：" + floderPath);
                return;
            }

            var fileName = Path.GetFileName(floderPath);
            var parentPath = Path.GetDirectoryName(floderPath);
            var dstZipPath = parentPath + "/" + fileName + ".zip";

            var beginTime = DateTime.Now;
            var result = ZipHelper.ZipFolder(floderPath, dstZipPath, false);
            var endTime = DateTime.Now;
            if (result)
            {
                EditorUtility.RevealInFinder(dstZipPath);
                Debug.Log(EPostProcessBuildRule.PackagingBuiltProjectFolderAsZip.Tr() + "成功: " + endTime.ToDefaultFormat() + ", 总时长: " + (endTime - beginTime).TotalSeconds + " 秒");

                //ZipHelper.UnzipFile(dstZipPath, parentPath + "/Unzip-" + fileName);
            }
            else
            {
                Debug.LogWarning(EPostProcessBuildRule.PackagingBuiltProjectFolderAsZip.Tr() + "失败: " + endTime.ToDefaultFormat() + ", 总时长: " + (endTime - beginTime).TotalSeconds + " 秒");
            }
        }
