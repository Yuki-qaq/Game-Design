...
namespace XCSJ.PluginCameras.States
{
    /// <summary>
    /// 相机控制器属性设置
    /// </summary>
    ...
    public class CameraControllerPropertySet : BasePropertySet<CameraControllerPropertySet>
    {
        ...
        public enum EPropertyName
        {
            ...
            /// </summary>
            [Name("恢复到原始状态")]
            [Tip("将相机控制器的变换恢复到程序启动时记录的状态", "Restore the transformation of the camera controller to the state recorded when the program is started")]
            ...
            /// </summary>
            [Name("恢复到上一次状态")]
            [Tip("将相机控制器的变换恢复到上一次记录的状态", "Restore the transformation of the camera controller to the last recorded state")]
            RecoverToLast,
        ...
        /// </summary>
        [Name("相机聚焦位置")]
        [Tip("仅在‘相机聚焦目标模式’为‘包围盒锚点’时生效", "Effective only when 'camera focus target mode' is' bounding box anchor '")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual, EPropertyName.TryFocusTarget)]
        [OnlyMemberElements]
        ...
        /// </summary>
        [Name("时间")]
        [Tip("聚焦到目标所需时间；单位：秒；", "Time required to focus on the target; Unit: second;")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual, EPropertyName.TryFocusTarget)]
        [OnlyMemberElements]
        ...
        /// </summary>
        [Name("时间")]
        [Tip("同步变换所需时间；单位：秒；", "Time required for synchronous transformation; Unit: second;")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual, EPropertyName.TrySyncTransfrom)]
        [OnlyMemberElements]
        ...
        /// 尝试变换到(通过朝向)规则
        /// </summary>
        public enum ETryTransformToByLookAtRule
        {
            /// <summary>
            /// 三维向量数据
            /// </summary>
        ...
        [EnumPopup]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual, EPropertyName.TryTransformToByLookAt)]
        public ETryTransformToByLookAtRule _tryTransformToByLookAtRule = ETryTransformToByLookAtRule.Vector3Data;

        /// <summary>
        /// 点和方向数据
        /// </summary>
        [Name("点和方向数据")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual | EValidityCheckType.Or, EPropertyName.TryTransformToByLookAt, nameof(_tryTransformToByLookAtRule), EValidityCheckType.NotEqual, ETryTransformToByLookAtRule.PointDirectionData)]
        ...
        /// </summary>
        [Name("朝向位置")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual | EValidityCheckType.Or, EPropertyName.TryTransformToByLookAt, nameof(_tryTransformToByLookAtRule), EValidityCheckType.NotEqual, ETryTransformToByLookAtRule.Vector3Data)]
        [OnlyMemberElements]
        ...
        /// </summary>
        [Name("时间")]
        [Tip("变换到所需时间；单位：秒；", "Change to the required time; Unit: second;")]
        [HideInSuperInspector(nameof(_propertyName), EValidityCheckType.NotEqual, EPropertyName.TryTransformToByLookAt)]
        [OnlyMemberElements]
        ...
        public override void Execute(StateData stateData, EExecuteMode executeMode)
        {
            ...
            switch (_propertyName)
            {
                ...
                case EPropertyName.TryTransformToByLookAt:
                    {
                        switch (_tryTransformToByLookAtRule)
                        {
                            case ETryTransformToByLookAtRule.Vector3Data:
                                {
                            ...
                            case ETryTransformToByLookAtRule.PointDirectionData:
                                {
                                    var mt = _TryTransformToByLookAt__mainTarget.GetValue();
                                    if (mt)
                                    {
                                        cameraController.TryTransformToByLookAt(mt, mt.position, _pointDirectionData.direction, Vector3.Distance(mt.position, _pointDirectionData.point), _TryTransformToByLookAt__time.GetValue());
                                    }
                                    break;
                                }
                        }
