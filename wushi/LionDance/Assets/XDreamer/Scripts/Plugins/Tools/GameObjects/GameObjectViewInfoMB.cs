...
namespace XCSJ.PluginTools.GameObjects
{
    ...
    /// </summary>
    [Name("游戏对象视角信息")]
    [Tip("游戏对象生成图像时可使用当前组件提供的信息", "When the game object generates an image, it can use the information provided by the current component")]
    [Tool(ToolsCategory.GameObject, rootType = typeof(ToolsManager))]
    [DisallowMultipleComponent]
    [XCSJ.Attributes.Icon(EIcon.Model)]
    [Owner(typeof(ToolsManager))]
    public class GameObjectViewInfoMB : InteractProvider
    {
        /// <summary>
        /// 图像规则
        /// </summary>
        ...
        public enum EImageRule
        {
            ...
            /// </summary>
            [Name("优先使用子对象视角信息")]
            [EnumFieldName("优先使用子对象视角信息")]
            [Tip("当子对象有当前组件时，优先使用子对象去获取图像", "When the sub object has the current component, it is preferred to use the sub object to obtain the image")]
            UseChildrenInfo = 1 << 1,
            ...
            [Name("拍摄")]
            [EnumFieldName("拍摄")]
            [Tip("使用当前对象配置信息来拍摄", "Use the current object configuration information to shoot")]
            Photograph = 1 << 2,
        }
        ...
        /// </summary>
        [Name("使用包围盒中心")]
        [Tip("当游戏对象包含粒子系统时，求出的包围盒是个包含粒子系统的大值，此时相机生成的图形可能为空", "When the game object contains a particle system, the calculated bounding box is a large value containing the particle system. At this time, the graphics generated by the camera may be empty")]
        ...
        public bool TryGetTexture(Camera camera, Vector2Int textureSize, out Texture2D texture)
        {
            ...
            {
                var infos = GetComponentsInChildren<GameObjectViewInfoMB>().ToList();
        ...
        public Texture2D CreateRenderTexture(Camera camera, Vector2Int textureSize)
        {
            ...
            if (camera && textureSize != Vector2Int.zero)
            {
                ...
                {
                    if (gameObject.TryGetComponent<BoundsProvider>(out var boundsProvider) 
                        && boundsProvider.TryGetBounds(out var bounds))
                    {
                ...
                var renderTexture = _constumCameraDetailInfo ? camera.Render(textureSize, center, _viewDistance, _viewAngle, gameObject) : camera.RenderWithBounds(textureSize, gameObject);
                rs = XGUIHelper.ToTexture2D(renderTexture);
        ...
        {
            if (gameObject.TryGetComponent<GameObjectViewInfoMB>(out var viewInfo) && viewInfo.enabled)
            {
            ...
            return camera.RenderWithBounds(textureSize, gameObject).ToTexture2D();
        }
    }
}
