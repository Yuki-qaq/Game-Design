    ...
    public class GUIItemList : BaseRepairmanGUI
    {
        ...
        protected void Awake()
        {
            ...
            guiItemPool.Init(
                () =>
                {
                    var go = itemButtonPrefab.XCloneObject();
                    if (go)
                    {
                        go.XSetParent(itemButtonPrefab.transform.parent);

                        if (prefabItem is GUIItemButton)
                        {
                            return go.GetComponent<GUIItemButton>();
                        }
                        else
                        {
                            return go.GetComponent<GUIItemToggle>();
                        }
                    }
