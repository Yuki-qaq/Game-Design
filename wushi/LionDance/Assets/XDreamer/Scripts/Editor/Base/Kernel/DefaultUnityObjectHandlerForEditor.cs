...
namespace XCSJ.EditorExtension.Base.Kernel
{
    ...
    public class DefaultUnityObjectHandlerForEditor : InstanceClass<DefaultUnityObjectHandlerForEditor>, IUnityObjectHandler
    {
        ...
        {
            UndoHelper.RegisterCreatedObjectUndo(objectToUndo);
        }
        ...
        public void DestoryModel(PluginCommonUtils.ComponentModel.Model model)
        {
            if (!model) return;
            UnityObjectHandlerEvent.CallDestroyObject(model.GetType(), model, () =>
            {
                UndoHelper.DestroyObjectWithRegisterObjectReferenceInScene(model);
            });
        }
        ...
        /// <exception cref="NotImplementedException"></exception>
        public void DestoryModels(IEnumerable<PluginCommonUtils.ComponentModel.Model> models)
        {
            if (models ==null) return;

            UndoHelper.DestroyModelsWithRegisterObjectReferenceInScene(models);
        }
        ...
        /// <returns></returns>
        public bool IsPartOfPrefabAsset(UnityEngine.Object componentOrGameObject) => PrefabUtility.IsPartOfPrefabAsset(componentOrGameObject);
        ...
        /// <returns></returns>
        public string ToJson<T>(IEnumerable<T> objects, GameObject relativeGameObject) where T : ScriptableObject => EditorHelper.ToJson(objects, relativeGameObject);
        ...
        /// <param name="relativeGameObject"></param>
        /// <returns></returns>
        public List<ScriptableObject> FromJson(string json, GameObject relativeGameObject) => EditorHelper.FromJson(json, relativeGameObject);
    }
}
