...
namespace XCSJ.PluginPhysicses.CNScripts
{
    ...
    public enum EScriptID
    {
        ...
        /// 获取物理关节当前刻度
        /// </summary>
        [ScriptName("获取物理关节当前刻度", nameof(GetPhysicsJointCurrentStep))]
        [ScriptDescription("获取物理关节当前刻度")]
        [ScriptReturn("成功返回 当前刻度值 ; 失败返回 #False ;")]
        [ScriptParams(1, EParamType.GameObjectComponent, "物理关节", typeof(PhysicsJoint))]
        #endregion
        GetPhysicsJointCurrentStep,

        #region 设置物理关节当前刻度
        /// <summary>
        /// 设置物理关节当前刻度
        /// </summary>
        [ScriptName("设置物理关节当前刻度", nameof(SetPhysicsJointCurrentStep))]
        [ScriptDescription("设置物理关节当前刻度")]
        [ScriptReturn("成功返回 #True ; 失败返回 #False ;")]
        [ScriptParams(1, EParamType.GameObjectComponent, "物理关节", typeof(PhysicsJoint))]
        [ScriptParams(2, EParamType.Int, "刻度")]
        #endregion
        SetPhysicsJointCurrentStep,
    }
}
