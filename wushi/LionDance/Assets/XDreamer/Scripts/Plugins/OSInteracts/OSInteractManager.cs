    ...
    public sealed class OSInteractManager : BaseManager<OSInteractManager>
    {
        ...
        public void OnOSSendMessageToUnity(string jsonString)
        {
            ...
            try
            {
                ...
                var osToUnityMsgCmdString = jsondata.objectValue[MsgCmd].ToString();
                var osToUnityMsgCmd = CommonFun.StringToEnum<EOSToUnityMsgCmd>(osToUnityMsgCmdString, EOSToUnityMsgCmd.None);
                switch (osToUnityMsgCmd)
                {
                    ...
                    case EOSToUnityMsgCmd.OSNotifyParams:
                        {
                            var notifyParams = jsondata[Msg][EVariableName.OSNotifyParams.ToString()];
                            string notifyParamsString = notifyParams.ToString();
                            scriptManager.varCollection.TrySetOrAddSetVarValue(EVariableName.OSNotifyParams.ToString(), notifyParamsString);
                            var varValue = "";
                            if (notifyParams.IsString && JsonData.FromJson(notifyParamsString) is JsonData paramJsonData && paramJsonData.IsObject)
                            {
                                varValue = paramJsonData.TryConvertToHierarchyJsonData().ToJson();
                            }
                            else if (notifyParams.IsObject)
                            {
                                varValue = notifyParams.TryConvertToHierarchyJsonData().ToJson();
                            }
                            else
                            {
                                varValue = notifyParamsString;
                            }
                            _osNotifyParamsVarString.TrySetOrAddSetHierarchyVarValue(varValue);
                            break;
                        }
                    default:
                        {
                ...
                scriptManager.ExecuteFunction(_alwaysCallbackFunc, osToUnityMsgCmdString);
                onOSToUnity?.Invoke(this, osToUnityMsgCmd, jsondata);
            }
            catch (Exception ex)
            {
        ...
        private void AnalogOSSendMessageToUnity(string msgCmd, JsonData msg)
        {
            ...
            if (showLogOfToUnity) Log.DebugFormat("模拟OS --> Unity消息命令: {0}", msgCmd);
        ...
        /// </summary>
        [Name("总是回调函数")]
        [Tip("当OS发送消息到Unity时，在内置处理完成后总是回调的函数；对应的消息命令消息存储在回调函数的传入参数中；", "When the OS sends a message to Unity, a function that always calls back after the built-in processing is completed; The corresponding message command message is stored in the incoming parameters of the callback function;")]
        [UserDefineFun]
        public string _alwaysCallbackFunc = "";

        /// <summary>
        /// 当OS到Unity
        /// </summary>
        public static event Action<OSInteractManager, EOSToUnityMsgCmd, JsonData> onOSToUnity;
        ...
        public string _osNotifyParamsVarString = "";
...
#endif
                        var key1 = (EVariableName)param[2];
                        var key2 = (EVariableName)param[4];
                        AnalogOSSendMessageToUnity((EOSToUnityMsgCmd)param[1],
                            key1 == EVariableName.None ? "" : key1.ToString(),
                            param[3].ToString(),
                            key2 == EVariableName.None ? "" : key2.ToString(),
                            param[5].ToString());
                        break;
                    }
            }
