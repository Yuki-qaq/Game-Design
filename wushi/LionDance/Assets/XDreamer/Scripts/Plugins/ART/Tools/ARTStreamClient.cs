...
namespace XCSJ.PluginART.Tools
{
    ...
    public class ARTStreamClient : InteractProvider
    {
        /// <summary>
        /// 服务器主机
        /// </summary>
        ...
        /// </summary>
        [Name("服务器端口")]
        [Tip("如仅接收数据本值为0即可；如使用DTrack2时，本端口值默认值为50105；", "If only data is received, this value can be 0; If DTrack2 is used, the default value of this port is 50105;")]
        ...
        /// </summary>
        [Name("数据超时")]
        [Tip("数据超时时间；单位：微秒；", "Data timeout; Unit: microsecond;")]
        ...
        /// </summary>
        [Name("服务超时")]
        [Tip("服务超时时间；单位：微秒；", "Service timeout; Unit: microsecond;")]
        ...
        /// </summary>
        [Name("ART坐标系统")]
        [Tip("ART服务器端使用的坐标系统", "Coordinate system used by ART server")]
        [EnumPopup]
    ...
    public abstract class BaseState
    {
        ...
        {
            var r = coordinateSystem.GetConvertMatrixToUnity();
    ...
    public class ARTFlyStickState : BaseState
    {
        ...
        /// 是按压的
        /// </summary>
        /// <param name="flystickButton"></param>
        /// <param name="deadZone"></param>
        /// <returns></returns>
        public bool IsPressed(Enum flystickButton,Vector2 deadZone)
        {
            if (TryGetValue(flystickButton, out var value))
            {
                if (value < deadZone.x) return false;
                if (value > deadZone.y) return false;
                return true;
            }
            return false;
        }
        ...
        /// <summary>
        /// 尝试获取绝对值：会根据死区对源值的绝对值做归一化处理；
        /// </summary>
        /// <param name="flystickButton">Flystick按钮枚举</param>
        /// <param name="value">为[0,1]区间的值</param>
        /// <param name="deadZone">死区</param>
        /// <returns></returns>
        public bool TryGetAbsValue(Enum flystickButton, out double value, Vector2 deadZone)
        {
            if (TryGetValue(flystickButton, out value))
            {
                value = Math.Abs(value).Normalization(deadZone.x, deadZone.y);
                return true;
            }
            value = 0;
            return false;
        ...
        /// <summary>
        /// 获取绝对值：会根据死区对源值的绝对值做归一化处理；
        /// </summary>
        /// <param name="flystickButton">Flystick按钮枚举</param>
        /// <param name="deadZone">死区值</param>
        /// <returns>结果为[0,1]区间的值</returns>
        public double GetAbsValue(Enum flystickButton, Vector2 deadZone)
        {
        ...
        /// <param name="deadZone">死区</param>
        /// <param name="dstValue">目标值</param>
        /// <returns></returns>
        public bool TryGetDstValue(Enum flystickButton, out double value, Vector2 deadZone, Vector2 dstValue)
        {
            if (TryGetValue(flystickButton, out value))
            {
                value = Math.Abs(value).ToDstValue(deadZone.x, deadZone.y, dstValue.x, dstValue.y);
                return true;
            }
            value = 0;
            return false;
        ...
        /// <param name="dstValue">目标值</param>
        /// <returns>结果为[0,1]区间的值</returns>
        public double GetDstValue(Enum flystickButton, Vector2 deadZone, Vector2 dstValue)
        {
            TryGetDstValue(flystickButton, out var value, deadZone, dstValue);
            return value;
        }
    }
}
