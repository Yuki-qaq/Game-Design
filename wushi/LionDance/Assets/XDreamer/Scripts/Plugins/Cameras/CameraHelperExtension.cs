...
using ERotateMode = XCSJ.PluginCameras.Controllers.ERotateMode;

namespace XCSJ.PluginCameras
{
    /// <summary>
    /// 相机组手扩展
    /// </summary>
    public static class CameraHelperExtension
    {
        ...
        /// 当前相机主控制器
        /// </summary>
        public static BaseCameraMainController currentCameraMainController
        {
            get
            {
                var cameraManager = CameraManager.instance;
                if (cameraManager)
                {
                    return cameraManager.GetCurrentCameraController();
                }
                return default;
            }
        }
        ...
        /// <summary>
        /// 创建渲染贴图:激活游戏对象及父级，并将目标游戏对象层级调整为相机渲染贴图层
        /// </summary>
        ...
        {
            if (camera && gameObject && textureSize != Vector2Int.zero)
            {
                // 记录游戏对象及子对象层
        ...
        /// <param name="gameObject"></param>
        /// <returns></returns>
        public static RenderTexture RenderWithBounds(this Camera camera, Vector2Int textureSize, GameObject gameObject)
        {
        ...
        public static RenderTexture Render(this Camera camera, Vector2Int textureSize, Vector3 viewCenter, float viewDistance, Vector3 viewOffsetAngle, GameObject gameObject)
        {
            ...
            try
            {
                ...
                return camera.Render(textureSize, gameObject);
            }
            finally
            {
                recorder.Recover();
        ...
        public static RenderTexture Render(this Camera camera, Vector2Int textureSize)
        {
            ...
            try
            {
                var cameraParentGameObjects = CommonFun.GetParentsGameObject(camera.gameObject, true);
                gameObjectRecorder.Record(cameraParentGameObjects);
                cameraParentGameObjects.ForEach(go => go.SetActive(true));// 启用父级游戏对象
