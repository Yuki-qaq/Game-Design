...
using XCSJ.PluginCameras.CNScripts;
using XCSJ.PluginCameras.Controllers;
using XCSJ.Scripts;
using XCSJ.Extension.Base.Maths;

namespace XCSJ.PluginCameras.Kernel
{
    /// <summary>
    /// 默认相机处理器
    /// </summary>
    public class DefaultCameraHandler : InstanceClass<DefaultCameraHandler>, ICameraHandler
    {
        ...
        public ReturnValue ExecuteScript(CameraManager manager, int id, ScriptParamList param)
        {
            switch ((ECameraScriptID)id)
            {
                ...
                case ECameraScriptID.TryTransformToByLookAt:
                    {
                        var name = param[1] as string;
                        var cameraController = ComponentCache.Get(typeof(BaseCameraMainController), true).components.FirstOrDefault(c => c.name == name) as BaseCameraMainController;
                        if (!cameraController) break;

                        var mainTarget = param[2] as GameObject;
                        if (!mainTarget) break;

                        var positionGO = param[3] as GameObject;
                        if (!positionGO) break;

                        var rotationGO = param[4] as GameObject;
                        if (!rotationGO) break;

                        var lookAtPosition = mainTarget.transform.position;
                        cameraController.TryTransformToByLookAt(mainTarget.transform, lookAtPosition, rotationGO.transform.eulerAngles, Vector3.Distance(lookAtPosition, positionGO.transform.position), (float)param[5]);
                        break;
                    }
