...
namespace XCSJ.PluginPeripheralDevice.XR.States
{
    ...
    public class XRInputDeviceEvent : Trigger<XRInputDeviceEvent>, IDropdownPopupAttribute
    {
        ...
        public override void OnEntry(StateData stateData)
        {
            ...
            TryGetCommonUsageState(out lastCommonUsageState, out _);
        }
        ...
        private bool TryGetCommonUsageState(out bool state, out object outValue)
        {
            state = false;
            outValue = 0;
            if (_eventType != EEventType.CommonUsages) return false;
            var inputDevice = InputDevices.GetDeviceAtXRNode(_xrNode);
            if (!inputDevice.isValid || string.IsNullOrEmpty(_commonUsages)) return false;
            ...
            if (usage is InputFeatureUsage<bool> usageBool)
            {
                ...
                {
                    state = _commonUsagesEventType == ECommonUsagesEventType.Value || !_deadZone.InDeadZone(value);
                    outValue = value;
                    return true;
                }
            }
        ...
        private bool lastCommonUsageState = false;
        ...
        {
            bool result = false;
            if (TryGetCommonUsageState(out var state, out var value))
            {
                switch (_clickType)
                {
                    case EClickType.DownAndUp:
                        {
                            if (lastCommonUsageState && !state) result = true;
                            break;
                        }
                    ...
                    case EClickType.Keeping:
                        {
                            if (lastCommonUsageState && state) result = true;
                            break;
                        }
                }
                lastCommonUsageState = state;
            }
        ...
        bool IDropdownPopupAttribute.TryGetOptions(string purpose, string propertyPath, out string[] options)
        {
            ...
            options = fieldInfos.Where(fi => !AttributeCache<ObsoleteAttribute>.Exist(fi)).Cast(fi => fi.Name).ToArray();
            return true;
        }
