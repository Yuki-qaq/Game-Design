...
namespace XCSJ.PluginXGUI
{
    ...
    [Name(XGUICategory.XGUI)]
    [Tip("用于调度、维护、管理GUI的插件", "Plug in for scheduling, maintenance and management GUI")]
    ...
    public sealed class XGUIManager : BaseManager<XGUIManager>
    {
        ...
        public override ReturnValue ExecuteScript(int id, ScriptParamList param)
        {
            switch ((EScriptID)id)
            {
                ...
                case EScriptID.GetCanvasControllerRenderMode:
                    {
                        var canvasCtrl = param[1] as CanvasController;
                        if (canvasCtrl)
                        {
                            return ReturnValue.Create(true, canvasCtrl.renderMode);
                        }
                        break;
                    }
                case EScriptID.SetCanvasControllerRenderMode:
                    {
                        var canvasCtrl = param[1] as CanvasController;
                        if (canvasCtrl)
                        {
                            switch (param[2] as string)
                            {
                                case "屏幕空间覆盖":
                                    {
                                        canvasCtrl.renderMode = RenderMode.ScreenSpaceOverlay;
                                        return ReturnValue.Yes;
                                    }
                                case "屏幕空间相机":
                                    {
                                        canvasCtrl.renderMode = RenderMode.ScreenSpaceCamera;
                                        return ReturnValue.Yes;
                                    }
                                case "世界空间":
                                    {
                                        canvasCtrl.renderMode = RenderMode.WorldSpace;
                                        return ReturnValue.Yes;
                                    }
                            }
                        }
        ...
        protected override void OnEnable()
        {
            ...
            this.XGetOrAddComponent<XGUIProvider>();
        }
    }
}
