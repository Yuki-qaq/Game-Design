...
namespace XCSJ.Extension.Base.Extensions
{
    ...
    public static class UnityObjectExtension
    {
        ...
        /// <param name="component">组件：如果本值有效，直接返回；否者设置本值并返回；</param>
        /// <param name="includeInactive">包含非激活的</param>
        /// <param name="isFirst">为True时获取第一个；为False时获取最后一个</param>
        /// <returns></returns>
        public static T XGetComponentInParent<T>(this Component mainComponent, ref T component, bool includeInactive = true, bool isFirst = true) where T : Component
        {
            if (!component && mainComponent)
            {
                var array = mainComponent.GetComponentsInParent<T>(includeInactive);
                var newComponent = isFirst ? array.FirstOrDefault() : array.LastOrDefault();
                if (newComponent) mainComponent.XModifyProperty(ref component, newComponent);
            }
            return component;
        }
        ...
        /// <param name="includeInactive">包含非激活的</param>
        /// <returns></returns>
        public static T XGetComponentInParentExcludeSelfGameObject<T>(this Component mainComponent, ref T component, bool includeInactive = true) where T : Component
        {
            if (!component && mainComponent)
            {
                var newComponent = mainComponent.GetComponentsInParent<T>(includeInactive).FirstOrDefault(c => c.gameObject != mainComponent.gameObject);
                if (newComponent) mainComponent.XModifyProperty(ref component, newComponent);
            }
            return component;
        }
        ...
        /// <param name="includeInactive">包含非激活的</param>
        /// <returns></returns>
        public static T XGetComponentInParentExcludeSelfComponent<T>(this Component mainComponent, ref T component, bool includeInactive = true) where T : Component
        {
            if (!component && mainComponent)
            {
                var newComponent = mainComponent.GetComponentsInParent<T>(includeInactive).FirstOrDefault(c => c != mainComponent);
                if (newComponent) mainComponent.XModifyProperty(ref component, newComponent);
            }
            return component;
        }
