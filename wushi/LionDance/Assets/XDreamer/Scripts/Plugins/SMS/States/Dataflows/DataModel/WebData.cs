...
namespace XCSJ.PluginSMS.States.Dataflows.DataModel
{
    ...
    public class WebData : Trigger<WebData>, ISerializationCallbackReceiver
    {
        ...
        /// </summary>
        [Name("URI")]
        [Tip("期望请求数据的网络URI路径", "Expected network URI path for requesting data")]
        public StringPropertyValue _uri = new StringPropertyValue();
        ...
        private void Request()
        {
            if (parent.isActive)
            {
                if (WebDataManager.Request(_uri.GetValue(), EDataType.Text, null, OnCompleted, OnFailed, null))
                {
                    //
                }
