...
namespace XCSJ.PluginCameras.CNScripts
{
    /// <summary>
    /// 相机脚本ID枚举
    /// </summary>
    ...
    public enum ECameraScriptID
    {
        ...
        /// 尝试变换到(通过朝向)
        /// </summary>
        [ScriptName("尝试变换到(通过朝向)", nameof(TryTransformToByLookAt))]
        [ScriptDescription("尝试变换到(通过朝向)")]
        [ScriptReturn("成功返回#True；失败返回#False；")]
        [ScriptParams(1, CameraControllerName_ScriptParam.ScriptParamType, "目标相机控制器对象名称:", typeof(BaseCameraMainController))]
        [ScriptParams(2, EParamType.GameObject, "主目标:")]
        [ScriptParams(3, EParamType.GameObject, "位置:")]
        [ScriptParams(4, EParamType.GameObject, "旋转:")]
        [ScriptParams(5, EParamType.FloatSlider, "时间:", 0f, 10f)]
