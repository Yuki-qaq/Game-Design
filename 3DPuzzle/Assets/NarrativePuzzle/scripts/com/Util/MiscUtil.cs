...
namespace com
{
    public class MiscUtil
    {
        ...
        public static List<T> ToList<T>(T[] inArray)
        {
            List<T> res = new List<T>();
            res.AddRange(inArray);
            return res;
        }

        public static T[] ToArray<T>(List<T> inList)
        {
            T[] res = inList.ToArray();
            return res;
        }

        /// <summary>
        /// pick n no repeat elements from list randomly
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="list"></param>
        /// <param name="n"></param>
        /// <returns></returns>
        public static List<T> PickNRandomFromList<T>(List<T> list, int n)
        {
            if (n > list.Count)
                n = list.Count;
            return (List<T>)list.OrderBy(x => Random.value).Take(n);
        }
        ...
        public static void LogGameObjectPath(GameObject go)
        {
            var g = go.transform;
            var res = "-LogGameObjectPath ";
            do
            {
                res += "-" + g.gameObject.name;
