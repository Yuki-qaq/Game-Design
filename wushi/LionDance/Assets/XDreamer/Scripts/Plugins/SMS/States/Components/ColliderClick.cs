...
namespace XCSJ.PluginSMS.States.Components
{
    ...
    [XCSJ.Attributes.Icon(EIcon.ColliderClick)]
    public class ColliderClick : Trigger<ColliderClick>
    {
        ...
        /// </summary>
        [Name("碰撞体交互命令数据列表")]
        [HideInSuperInspector(nameof(_responseInteractCmd), EValidityCheckType.False)]
        public List<ColliderInteractCmdData> _colliderInteractCmdDatas = new List<ColliderInteractCmdData>();

        /// <summary>
        /// 碰撞体交互命令数据
        /// </summary>
        [Serializable]
        public class ColliderInteractCmdData
        {
            /// <summary>
            /// 交互命令名称
            /// </summary>
            ...
            /// 是否匹配
            /// </summary>
            /// <param name="interactCmdName"></param>
            /// <param name="clickType"></param>
            /// <returns></returns>
            public bool IsMatch(string interactCmdName, EClickType clickType)
            {
                return _interactCmdName == interactCmdName && _clickType == clickType;
            }
        }
        ...
        public override void Reset()
        {
            ...
            _colliderInteractCmdDatas.Add(new ColliderInteractCmdData());
        }
        ...
        {
            if (!finished && interactData.interactable)
            {
                if (_colliderInteractCmdDatas.Exists(d => d.IsMatch(interactData.cmdName, clickType)))
                {
        ...
        private void InitCollider()
        {
            ...
            goList.ForEach(go=>
            {
                if (go)
                {
                    ...
                    {
                        if (c.gameObject == go || !c.GetComponent<InteractableEntity>())
                        {
                            colliders.Add(c);
                        }
                    }
