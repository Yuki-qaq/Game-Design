...
namespace XCSJ.PluginTools.Draggers
{
    ...
    public class AnalogMouseInputController : Interactor, IAnalogInputUpdater
    {
        ...
        /// 射线变换更新规则
        /// </summary>
        public enum ERayTransformUpdateRule
        {
            /// <summary>
            /// 无
            /// </summary>
            ...
            /// </summary>
            [Name("同步自定义射线原点变换位置")]
            SyncCustomRayOrginTransformPosition,
        }
        ...
        [Name("射线变换更新规则")]
        [EnumPopup]
        public ERayTransformUpdateRule _rayTransformUpdateRule = ERayTransformUpdateRule.None;

        /// <summary>
        /// 射线变换更新规则
        /// </summary>
        public ERayTransformUpdateRule rayTransformUpdateRule { get => _rayTransformUpdateRule; set => _rayTransformUpdateRule = value; }
        ...
        [Name("自定义射线原点变换")]
        [ValidityCheck(EValidityCheckType.NotNull)]
        [HideInSuperInspector(nameof(_rayTransformUpdateRule), EValidityCheckType.NotEqual, ERayTransformUpdateRule.SyncCustomRayOrginTransformPosition)]
        public Transform _customRayOrginTransform;
        ...
        [Name("自定义射线原点偏移量")]
        [HideInSuperInspector(nameof(_rayTransformUpdateRule), EValidityCheckType.NotEqual, ERayTransformUpdateRule.SyncCustomRayOrginTransformPosition)]
        public Vector3PropertyValue _customRayOrginOffset = new Vector3PropertyValue();
        ...
        protected void Update()
        {
            if (rayTransform)
            {
                switch (_rayTransformUpdateRule)
                {
                    case ERayTransformUpdateRule.SyncCustomRayOrginTransformPosition:
                        {
                            if (_customRayOrginTransform)
                            {
                                rayTransform.position = _customRayOrginTransform.position;
                                if (_customRayOrginOffset.TryGetValue(out var offset))
                                {
                                    rayTransform.position += _customRayOrginTransform.forward * offset.z + _customRayOrginTransform.up * offset.y + _customRayOrginTransform.right * offset.x;
                                }
                            }
                            break;
                        }
        ...
        /// </summary>
        [Name("射线变换横向转动输入速度")]
        public float _horizontalInputSpeed = 1;
        ...
        /// </summary>
        [Name("射线变换纵向转动输入速度")]
        public float _verticalInputSpeed = 1;
        ...
        public void UpdateAxis(IInput input, string name, float value)
        {
            ...
            {
                horizontalInputValue = value * _horizontalInputSpeed;
            }

            if (_verticalInput == name)
            {
                verticalInputValue = value * _verticalInputSpeed;
            }
        }
