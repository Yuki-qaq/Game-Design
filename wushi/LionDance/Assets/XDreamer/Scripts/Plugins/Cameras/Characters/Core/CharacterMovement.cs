...
namespace XCSJ.PluginCameras.Characters
{
    /// <summary>
    /// Character Movement.
    /// 
    ...
    /// </summary>
    [Name("角色运动")]
    [Tip("执行所有与角色移动相关的工作(也称为角色发动机）。例如：附加力、冲力、限制(约束)，平台交互等", "Perform all work related to character movement (also known as character engine). For example: additional force, impulse, restriction (constraint), platform interaction, etc")]
    [RequireComponent(typeof(Rigidbody), typeof(GroundDetection))]
    public sealed class CharacterMovement : BaseCharacterMB
    {
        #region 变换
        ...
        [Name("最大横向速度")]
        [Tip("此角色可以移动的最大横向速度，包括外力的运动，如滑动、碰撞等。", "The maximum lateral speed that this character can move, including the movement of external forces, such as sliding, collision, etc.")]
        [SerializeField]
        private float _maxLateralSpeed = 10.0f;

        [Name("最大上升速度")]
        [Tip("沿Y轴上移的最大速度，包括外力的运动，如滑动、碰撞等。", "The maximum speed of moving up along the y-axis, including the movement of external forces, such as sliding, collision, etc.")]
        [SerializeField]
        private float _maxRiseSpeed = 20.0f;

        [Name("最大下落速度")]
        [Tip("沿Y轴下移的最大速度，包括外力的运动，如滑动、碰撞等。", "The maximum speed of moving down along the Y axis, including the movement of external forces, such as sliding, collision, etc.")]
        [SerializeField]
        private float _maxFallSpeed = 20.0f;

        [Name("使用重力")]
        [Tip("启用/禁用角色的自定义重力；如果启用，角色会受到自定义重力的影响。", "Enable / disable the custom gravity of the character; When on, the character is affected by custom gravity.")]
        [SerializeField]
        private bool _useGravity = true;

        [Name("重力")]
        [Tip("此角色的重力大小；通过此字段手动设置重力，来进行更细致的调整和控制。", "The gravity of this character; Set gravity manually through this field for more detailed adjustment and control.")]
        [SerializeField]
        private float _gravity = 25.0f;

        [Name("陡坡滑坡")]
        [Tip("标识角色是否应该从陡峭的斜坡上滑下来", "Identifies whether the character should slide down a steep slope")]
        [SerializeField]
        private bool _slideOnSteepSlope = false;

        [Name("坡度限制")]
        [Tip("可行走斜坡的最大角度（单位:角度）；当坡度大于此值时，角色会滑下斜坡；当坡度小于此值时，角色会停留在斜坡上，不会下滑。", "Maximum angle of walking slope (unit: angle); When the slope is greater than this value, the character will slide down the slope; When the slope is less than this value, the character will stay on the slope and will not slide down.")]
        [SerializeField]
        [Range(0, 89)]
        private float _slopeLimit = 45.0f;

        [Name("滑动重力倍数")]
        [Tip("从陡坡上滑下来时所施加的重力。", "The force of gravity exerted when sliding down a steep slope.")]
        [SerializeField]
        private float _slideGravityMultiplier = 2.0f;

        [Name("捕捉地面")]
        [Tip("启用后，将强制角色安全地跟随可行走的'地面'几何体。", "When on, forces the character to safely follow the walkable 'ground' geometry.")]
        [SerializeField]
        private bool _snapToGround = true;

        [Name("捕捉强度")]
        [Tip("对离'地面'有多近保持角色的容忍度。0不捕捉,1贴地.", "Be tolerant of how close you are to the ground. 0 does not snap, 1 is close to the ground")]
        [Range(0.0f, 1.0f)]
        [SerializeField]
        ...
        {
            get { return Vector3.Dot(velocity, characterTransform.forward); }
        }
        ...
        {
            var p = characterTransform.position;
            var q = characterTransform.rotation;
        ...
        /// <param name="onlyLateral"></param>
        public void RotateTo(Vector3 direction, bool onlyLateral = true)
        {
            if (onlyLateral)
                direction.y = 0.0f;
        ...
        private void ApplyMovement(Vector3 desiredVelocity, float maxDesiredSpeed, bool onlyLateral)
        {
            ...
            var dot = Vector3.Dot(velocity, groundPoint - characterTransform.position);
            if (dot <= 0.0f)
                return;
        ...
        private void SnapToGround()
        {
            ...
            if (isOnLedgeSolidSide)
                distanceToGround = Mathf.Max(0.0f, Vector3.Dot(characterTransform.position - groundPoint, characterTransform.up) - groundOffset);

            // Compute final snap velocity and update character's velocity

            var snapVelocity = characterTransform.up * (-distanceToGround * snapStrength / Time.deltaTime);
    ...
    /// 基础角色组件
    /// </summary>
    public abstract class BaseCharacterMB : InteractProvider { }
}
