...
namespace XCSJ.PluginTools.Inputs
{
    ...
    public abstract class RayInput<TRayCmd> : BaseInput<TRayCmd> where TRayCmd : RayCmd, new()
    {
        ...
        [Name("射线规则")]
        [EnumPopup]
        [Group("射线设置", textEN ="Ray Settings", defaultIsExpanded = false)]
        ...
        [EnumPopup]
        [HideInSuperInspector(nameof(_rayRule), EValidityCheckType.NotEqual, ERayRule.UseRay)]
        public ERayHitObjectRule _rayHitObjectRule = ERayHitObjectRule.None;
        ...
        [HideInSuperInspector(nameof(_rayRule), EValidityCheckType.NotEqual | EValidityCheckType.Or, ERayRule.UseRay, nameof(_rayHitObjectRule), EValidityCheckType.NotEqual, ERayHitObjectRule.MaxDistanceLimit)]
        public Transform _maxDistanceOrigin;
        ...
        /// 最大距离原点
        /// </summary>
        public Transform maxDistanceOrigin => _maxDistanceOrigin ? _maxDistanceOrigin : transform;

        private float maxDistanceSqr => _maxDistance * _maxDistance;
        ...
        private bool RayHitObjectValid(Transform hitObject)
        {
            switch (_rayHitObjectRule)
            {
                case ERayHitObjectRule.MaxDistanceLimit:
                    {
                        return (hitObject.position - maxDistanceOrigin.position).sqrMagnitude <= maxDistanceSqr;
                    }
            }
            return true;
        }
                ...
                case ERayRule.UseRay:
                    {
                        ...
                        if (hitRay.HasValue && _rayHitter.TryGetHit(hitRay.Value, out var hit))
                        {
                            ...
                            if(RayHitObjectValid(hit.collider.transform))
                            {
                ...
                case ERayHitObjectRule.MaxDistanceLimit:
                    {
                        ...
                        Gizmos.DrawSphere(maxDistanceOrigin.position, _maxDistance);
