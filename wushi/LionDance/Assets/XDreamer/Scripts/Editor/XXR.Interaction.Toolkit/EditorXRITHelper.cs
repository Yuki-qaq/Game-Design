...
using XCSJ.PluginXXR.Interaction.Toolkit.Tools.Locomotion;
...
namespace XCSJ.EditorXXR.Interaction.Toolkit
{
    ...
    public static class EditorXRITHelper
    {
        ...
        /// <param name="defaultMainCameraAction"></param>
        /// <returns></returns>
        public static MonoBehaviour Create(out CameraController hmd, out Transform leftHand, out Transform rightHand, out Transform locomotionSystem, Func<CameraController> hmdFunc = null,Action<Transform, CameraController> defaultMainCameraAction = null)
        {
            hmd = default;
            leftHand = default;
            rightHand = default;
            ...
            if (defaultMainCameraAction == null)
            {
                defaultMainCameraAction = (c, cc) =>
                {
                    //销毁Unity默认创建的HMD
                    if (c)
                    {
            ...
            //处理Unity默认创建的HMD
            defaultMainCameraAction(defaultMainCamera, hmd);
            ...
            //HMD相机事件
            var cameraControllerEvent = hmd.XGetOrAddComponent<CameraControllerEvent>();
            cameraControllerEvent.Add(ECameraControllerEvent.OnSwitchedToCurrent, origin.gameObject, true);
            cameraControllerEvent.Add(ECameraControllerEvent.OnWillSwitchToLast, origin.gameObject, false);
            ...
            if (leftHand)
            {
                ...
                if (analogController)
                {
                    analogController.XModifyProperty(() => {
                        var actionName = EActionName.LeftHand.Tr();
                        analogController._poseActionName = actionName;
                        analogController._interactActionNames.Add(actionName);
                        analogController._customActionNames.Add(EActionName.MainMenu.Tr());
                    });                    
                }
            }
            else
            ...
            if (rightHand)
            {
                ...
                {
                    analogController.XModifyProperty(() => {
                        var actionName = EActionName.RigthHand.Tr();
                        analogController._poseActionName = actionName;
                        analogController._interactActionNames.Add(actionName);
                        analogController._customActionNames.Add(EActionName.MainMenu.Tr());
                    });
...
#endif//end XDREAMER_XR_INTERACTION_TOOLKIT_2_0_0_OR_NEWER
#endif//end XDREAMER_XR_INTERACTION_TOOLKIT
        ...
        public static MonoBehaviour CreateUnityDefault(out Transform cameraOffset, out Transform mainCamera, out Transform leftHand, out Transform rightHand, bool addInputActioAsset = false)
        {
            ...
            if (!es)
            {
                ...
                if (!es)
                {
                    es = EditorHelper.ExecuteMenuItem<EventSystem>("GameObject/UI/Event System");
                }                
            }

            if (es)
            {
                es.GetComponent<StandaloneInputModule>().XDestoryObject();
                es.XGetOrAddComponent<XRUIInputModule>();
            }
