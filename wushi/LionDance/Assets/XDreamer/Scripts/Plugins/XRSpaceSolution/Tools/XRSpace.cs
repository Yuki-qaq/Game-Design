...
namespace XCSJ.PluginXRSpaceSolution.Tools
{
    ...
    public class XRSpace : InteractProvider
    {
        ...
        /// <summary>
        /// 当前:如果同时有多个启用的此类型组件，则存储第一个启用的；
        /// </summary>
        public static XRSpace current { get; private set; }
        ...
        protected override void OnEnable()
        {
            ...
            {
                Debug.LogWarning("游戏对象[" + current.gameObject.ObjectToString() + "]存在已启用的XR空间组件，与游戏对象[" + gameObject.ObjectToString() + "]上的XR空间组件可能冲突！");
                return;
            }
            current = this;
        }
        ...
        protected override void OnDisable()
        {
            ...
            if (current == this) current = default;
        }
        ...
        private void OnXRAnswerReceived(XRAnswer answer)
        {
            ...
            {
                OnHandleXRSpaceConfigA(xRSpaceConfigA);
            }
            else if(answer is DelayFunctionA delayFunctionA)
            {
                CommonFun.DelayCall(() =>
                {
                    ...
                    if (scriptManager)
                    {
                        scriptManager.ExecuteScripts(delayFunctionA.scriptStrings);
                    }
                }, delayFunctionA.delayTime);
            }
        }
        ...
        private void OnHandleXRSpaceConfigA(XRSpaceConfigA xRSpaceConfigA)
        {
            ...
            //左手控制器
            var leftController = rigOwner.leftController;
            if (leftController)
            {
                var interactorVisual = leftController.GetComponent<XRInteractorVisual>();
                if (interactorVisual)
                {
                    interactorVisual.currentModelName = xRSpaceConfigA.leftController.modelName;
                }
            }
            ...
            //右手控制器
            var rightController = rigOwner.rightController;
            if (rightController)
            {
                var interactorVisual = rightController.GetComponent<XRInteractorVisual>();
                if (interactorVisual)
                {
                    interactorVisual.currentModelName = xRSpaceConfigA.rightController.modelName;
                }
            }
