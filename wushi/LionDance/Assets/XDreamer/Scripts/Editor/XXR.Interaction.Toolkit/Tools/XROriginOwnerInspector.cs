...
namespace XCSJ.EditorXXR.Interaction.Toolkit.Tools
{
    ...
    public class XROriginOwnerInspector : MBInspector<XROriginOwner>
    {
        ...
        /// 无动作资产文本
        /// </summary>
        public const string NoActionAssetText = "No preset action resources were found in the project for the [InputActionManager] type, Standard assets can be imported from the [XR Interaction Toolkit]";
        ...
        [LanguageTuple(NoActionAssetText, "项目中未找到适用于[输入动作管理器]类型的预设动作资源,可从[XR Interaction Toolkit]中导入标准资产")]
        private void DrawSetPreset()
        {
            ...
            // 设置[右手控制器]预设动作
            {
                ...
                EditorGUI.BeginDisabledGroup(!rightController);
                {
                    ...
                    {
                        UICommonFun.RichHelpBox(@"<size=12>" + NoActionAssetText.Tr(typeof(XROriginOwnerInspector)) + "</size>", MessageType.Warning);
                    }
                }
                EditorGUI.EndDisabledGroup();
            }
