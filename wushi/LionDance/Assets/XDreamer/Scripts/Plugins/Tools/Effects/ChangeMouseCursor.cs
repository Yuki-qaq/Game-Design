        ...
        public override void EnableEffect(InteractData interactData, GameObject gameObject)
        {
            switch (_cursorType)
            {
                ...
                case ECursorType.ImageOfUGUI:
                    {
                        activeImageOfUGUI = true;
                        break;
                    }
            }
        }

        private bool activeImageOfUGUI = false;

        private void ActiveImageOfUGUI()
        {
            if (activeImageOfUGUI && imageRectTransform)
            {
                if (_image && GetEffectData(gameObject) is InteractPropertyData interactPropertyData)
                {
                    var texture = interactPropertyData?.texture;
                    if (texture)
                    {
